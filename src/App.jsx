...
function Header(props) {
  ...
  const [menuHidden, setMenuHidden] = useState(1)
  return (
    <>
      <header>
        <div className='header'>
          <div className='logo_and_links'>
            <FaBars className='bars' onClick={() => setMenuHidden(false)} />
            <div className={menuHidden ? 'mobile-menu hidden' : 'mobile-menu'}>
              <HiX className='close' onClick={() => setMenuHidden(true)} />
          ...
          <div className='cart_and_user'>
            <div className='cart_logo' onClick={() => setHidden(!hidden)}>
              <div
                className={`items ${
                  props.items === 0 || !props.submitted ? 'hidden' : ''
                }`}
              >
                {props.items}
              </div>
              <svg width='22' height='20' xmlns='http://www.w3.org/2000/svg'>
                <path
                  ...
                  fill={`${hidden ? '#69707D' : '#000'}`}
                  fill-rule='nonzero'
                />
              </svg>
            </div>
            <img src={`${process.env.PUBLIC_URL}/image-avatar.png`} alt='123' />
          </div>
        </div>
      </header>
      <div className={`cart ${hidden ? 'hidden' : ''}`}>
        ...
        <div className='detail'>
          {props.items === 0 || !props.submitted ? (
            <p className='detail-p'>your cart is empty</p>
          ) : (
            <div className='info'>
              <div className='item-price'>
                <img src={image_product_1_thumbnail} alt='product_1' />
                <div className='item-price-text'>
                  <p>Fall Limited Edition Sneakers</p>
                  <p>
                    $125.00 x {props.items}
                    <span>{`   $${125 * props.items}`}.00</span>
                  </p>
                </div>
                <svg
                  width='14'
                  ...
                  fill={isHovered ? 'hsl(219, 9%, 45%)' : '#C3CAD9'}
                >
                  <defs>
                    <path
        ...
        <div className='section'>
          <div className='images'>
            <div className='main-img'>
              <img src={largeImages[activeImageIndex]} alt='product' />
              <FaChevronLeft
                className='left'
                onClick={() => {
                  setActiveImageIndex(
                    (activeImageIndex - 1 + tinyImages.length) %
                      tinyImages.length
                  )
                }}
              />
            ...
            <div className='thumbnails'>
              {tinyImages.map((img) => {
                return (
                  <div
                    className={
                      activeImageIndex === tinyImages.indexOf(img)
                        ? 'thumbnail active'
                        : 'thumbnail'
                    }
                  >
                    <img
                      ...
                      alt=''
                      onClick={() => {
                        setActiveImageIndex(tinyImages.indexOf(img))
                      }}
                    />
                  </div>
                )
          ...
          <div className='desc'>
            <div className='desc-content'>
              ...
              <p>
                ...
                Consequatur odio nulla ab, blanditiis veniam, voluptatum fugiat
                perspiciatis alias voluptates reprehenderit.
              </p>
              ...
              <div className='btns'>
                <div className='plus-minus'>
                  <FaMinus
                    className='minus'
                    onClick={() => {
                      setItems((item) => (item > 0 ? item - 1 : (item = 0)))
                    }}
                  />
                  <span className='zero'>{items}</span>
                  <FaPlus
                    className='plus'
                    onClick={() => {
